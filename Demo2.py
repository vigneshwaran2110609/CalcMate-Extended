...
# Main input and processing area
try:
    st.write("")
    ...
    # Add two solve buttons
    solve_with_gpt = st.button("Solve using GPT")
    solve_with_llama = st.button("Solve using LLaMA")
    ...
    suffix = """Now, convert the following word problem into a math equation with clear, numbered steps:
                    ...
                    3. Provide reasoning for each equation formation. But DO NOT solve the equation mathematically step by step.
    ...
    if solve_with_gpt:
        ...
        steps = standardize_output(steps)
        st.write(steps)
        st.write("Therefore after solving the equations we get, ")
        solution_text = "<br>".join(
            [f"The value of {var} ( {variable_labels[var]} ) is {solution_values.get(var, 'unknown')}." for var in variable_labels]
        )
        st.markdown(solution_text, unsafe_allow_html=True)
